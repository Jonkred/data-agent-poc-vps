...
until curl -sf "$CH/ping" > /dev/null 2>&1; do
  printf "."; sleep 3
done
echo " ✔"

run_sql() {
  curl -sf "$CH/" -u "$AUTH" --data-binary "$1" > /dev/null || {
    echo "  ERRO na query: $(echo $1 | head -c 80)..."
    return 1
  }
}
...
curl -sf "$CH/?database=poc_dw" -u "$AUTH" \
  --data-binary "SHOW TABLES FORMAT PrettyCompact"
